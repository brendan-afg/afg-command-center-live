...
function renderCommand() {
  ...
  $("#source-banner").innerHTML = operatingState.locked
    ...
    : `<div><strong>${escapeHtml(source.name)} · authenticated Drive API</strong><p>${source.scannedFolders} folders returned by the configured authenticated scope · ${source.failedFolderCount} folder failures · ${source.evidenceReadErrorCount} evidence failures</p></div><div class="source-meta">Updated ${escapeHtml(formatDate(state.generatedAt))}<br>Run ${escapeHtml(state.manifest.runId)}</div>`;
